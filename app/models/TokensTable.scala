...
trait TokensTable {
   ...
   *  @param createdAt Database column created_at SqlType(varchar), Length(16,true), Default(None) */
  case class TokensRow(userId: Option[Long] = None, token: Option[String], createdAt: Option[String] = None)
  ...
  implicit def GetResultTokensRow(implicit e0: GR[Option[Long]], e1: GR[Option[String]]): GR[TokensRow] = GR{
    prs => import prs._
    TokensRow.tupled((<<?[Long], <<?[String], <<?[String]))
  }
  ...
  class Tokens(_tableTag: Tag) extends profile.api.Table[TokensRow](_tableTag, "tokens") {
    def * = (userId, token, createdAt).<>(TokensRow.tupled, TokensRow.unapply)
    ...
    val token: Rep[Option[String]] = column[Option[String]]("token", O.Length(255,varying=true))
    /** Database column created_at SqlType(varchar), Length(16,true), Default(None) */
    val createdAt: Rep[Option[String]] = column[Option[String]]("created_at", O.Length(16,varying=true), O.Default(None))
