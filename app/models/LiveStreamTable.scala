...
trait LiveStreamTable {
  ...
  case class LiveStreamRow(id: Long, title: Option[String] = None, description: Option[String] = None, views: Option[Int] = Some(0), likes: Option[Int] = Some(0), dislikes: Option[Int] = Some(0), userId: Long, postingDate: Option[String] = None, path: String)
  /** GetResult implicit for fetching LiveStreamRow objects using plain SQL queries */
  implicit def GetResultLiveStreamRow(implicit e0: GR[Long], e1: GR[Option[String]], e2: GR[Option[Int]], e3: GR[String]): GR[LiveStreamRow] = GR{
    prs => import prs._
    LiveStreamRow.tupled((<<[Long], <<?[String], <<?[String], <<?[Int], <<?[Int], <<?[Int], <<[Long], <<?[String], <<[String]))
  }
  ...
  class LiveStream(_tableTag: Tag) extends profile.api.Table[LiveStreamRow](_tableTag, "live_stream") {
    def * = (id, title, description, views, likes, dislikes, userId, postingDate, path).<>(LiveStreamRow.tupled, LiveStreamRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), title, description, views, likes, dislikes, Rep.Some(userId), postingDate, Rep.Some(path))).shaped.<>({r=>import r._; _1.map(_=> LiveStreamRow.tupled((_1.get, _2, _3, _4, _5, _6, _7.get, _8, _9.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
